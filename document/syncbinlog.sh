...
BINLOG_BASENAME=$(mysql --login-path=default -Bse "SHOW GLOBAL VARIABLES LIKE 'log_bin_basename'")
if [[ $? -eq "1" ]]; then
...
BINLOG_INDEX_FILE=`mysql --login-path=default -Bse "SHOW GLOBAL VARIABLES LIKE 'log_bin_index'" | tail -1 | awk '{ print $2 }'`
...
do
    ...
    mysqlbinlog --login-path=default  \
