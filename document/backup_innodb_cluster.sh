...
getPrimary() {
     PRIMARY=$(mysql --login-path=default -BN -h $hostname -e "select member_host from performance_schema.replication_group_members where member_role='PRIMARY' limit 1" 2>/dev/null)  
     if [ $? -ne 0 ]
     then
...
checkTable() {
   mysql --login-path=default -B -h $hostname -e "select * from information_schema.tables 
              where table_schema='mysql' and table_name=
              'backup_group' limit 1" | grep backup_group >/dev/null
   if [ $? -ne 0 ] 
   then
      mysql --login-path=default -B -h $primary -e "create table mysql.backup_group( 
...
checkView() {
  mysql --login-path=default -B -h $hostname -e "select count(*) from information_schema.tables 
             where table_type='VIEW' 
             and table_schema='sys' and 
             table_name='gr_member_routing_candidate_status'" | grep '1' >/dev/null
  if [ $? -ne 0 ]
  then
     mysql --login-path=default -B -h $primary -e "USE sys;
...
getTheBackup() {
  mysql --login-path=default -BN -h $primary -e "set group_replication_consistency='BEFORE_AND_AFTER';
                            INSERT INTO 
...
removeLock() {
  mysql --login-path=default -BN -h $primary -e "set group_replication_consistency='BEFORE_AND_AFTER';                            delete from mysql.backup_group"
}

takeTheBackup() {
  hostname_with_lock=$(mysql --login-path=default -BN -h $hostname -e "SELECT hostname from mysql.backup_group;")
  if [ "$hostname_with_lock" == "$hostname" ]
  then
    echo "BACKUP is performed on $hostname"
    date=$(date '+%Y-%m-%d')
    mysqldump --login-path=default -h$hostname --master-data=2 --flush-logs --single-transaction --add-drop-table --all-databases --triggers --routines --events > /backup/vcmc-$hostname-$date.sql
...
checkStatus() {
  read candidate readonly applyqueue certqueue < <(mysql --login-path=default -BN -h node3 -e "select * from sys.gr_member_routing_candidate_status;")
   if [ "$candidate" == "NO" ]
   then
